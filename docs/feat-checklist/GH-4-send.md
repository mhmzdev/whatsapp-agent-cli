...
**FINDING-04 · Minor · `whatsapp_agent/client.py`** — a send that splits into five parts and fails on part three leaves the first two delivered, with no way to resume. That is inherent to a message API with no transaction, and the ids of the delivered parts are already in the store; #5 should not try to solve it, but a caller retrying a long send will duplicate the early parts.
