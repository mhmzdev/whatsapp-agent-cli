...
# feat: recv — long-poll with cursor after batch, dedup and JSON output          ⬜ BACKLOG
...
- [ ] A message sent from the phone appears on stdout as one JSON object per line — `verify: python3 tests/smoke.py` (fake session) plus the manual phone step
- [ ] The cursor advances only after the whole batch is printed and recorded; a crash mid-batch redelivers rather than skips — `verify: python3 tests/smoke.py`
- [ ] A message id already in the store is never printed again — `verify: python3 tests/smoke.py`
- [ ] A first run with no cursor asks for new traffic only; `--replay` asks for the backlog — `verify: python3 tests/smoke.py` asserts the request's params in both cases
- [ ] 409 exits 9 naming the other poller, in one-shot and in `--follow` — `verify: python3 tests/smoke.py`
- [ ] 401, or 400 with `error.code` 100, exits 4 immediately and polls no further — `verify: python3 tests/smoke.py`
- [ ] In `--follow`, a 503 backs off exponentially and recovers; in one-shot it exits 7 — `verify: python3 tests/smoke.py` with an injected clock, asserting simulated sleep, never real
- [ ] Polls are paced at 15/min and statuses at 12/min, each its own window — `verify: python3 tests/smoke.py`
- [ ] `--typing` marks each delivered message read and shows the indicator; without it, nothing is sent to `/statuses` — `verify: python3 tests/smoke.py`
- [ ] The creator is recorded from the first inbound message, after which `send` works without `--to` — `verify: python3 tests/smoke.py`
- [ ] Ctrl-C during `--follow` exits 130 with the cursor where the last complete batch left it — `verify: python3 tests/smoke.py`
