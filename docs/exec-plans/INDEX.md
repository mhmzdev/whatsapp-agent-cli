...
|---|---|---|
| [GH-44-dotenv](completed/GH-44-dotenv.md) | 2026-09-20 | Every command reads `./.env` (the CLI only; the library never does) and fills gaps, the shell always wins and a blank export is a gap; the token order is shell, `--token-file`, `./.env`; one `env:` stderr line names each variable's source, only when `./.env` mattered; bad lines are skipped by number; doctor shows each variable's source; `--no-env-file` is the old behaviour word for word; `make up` and `make live` let the CLI read `.env` itself |
